...
- 最小变化原则（v3.8.5）：字段仅在有明确证据时更新，否则严格复制上一状态
- 世界书角色读取 + 好感度系统集成
- 用户角色排除 + 泛称条目过滤
- 重要记忆评分排序管理
- 手动点击编辑字段
...
**核心约束（v3.8.5 更新为最小变化原则）：**
- 每个字段仅在本轮正文有**明确证据**时才更新，否则**严格复制上一状态**，禁止凭空修改（取代旧「必须不同」约束）
- 变化必须符合聊天内容的**线性逻辑**
- 细项规则：时间精确到分钟按事件推进（对话几分钟/战斗几十分钟/旅行数小时天，无流逝不变，用户给定以用户为准）；区域仅明确移动时更新（层级格式）；在场+BUFF 仅明确加入/离开/获得BUFF时更新（伤口心态衣服不算BUFF）；不在场仅随在场变化；处女膜仅破处性交时更新；做爱次数仅性交射精时+1；好感度仅明确变化时更新（参照世界书系统）；记忆仅新事件追加每角色≤6条合计≤70字；外貌仅换装/脱衣/发型变化时更新
