(function () {
  ...
  var WST_VERSION = '3.8.5'; // 版本号：更新后首次使用自动清理旧数据
  ...
  function buildStatePrompt(state) {
    ...
    lines.push('5. 在场角色+BUFF：仅当角色明确加入/离开/获得BUFF才更新，否则严格复制。格式：琴（回溯魔法）。BUFF仅指永久/临时正向/负向收益（如回溯魔法、中毒、力量增强）；伤口、心态、衣服等不算BUFF。');
    lines.push('6. 不在场角色：仅当在场角色变化导致不在场列表变化时才更新。格式：角色名-在做什么（≤10字）。当前场景中的角色绝不能误判为不在场。');
    ...
    lines.push('在场角色+BUFF：琴（回溯魔法）|芭芭拉（治疗中）');
    lines.push('不在场角色：诺艾尔-在骑士团值班');
    lines.push('处女膜状态：完好');
    lines.push('做爱次数：0次');
  ...
  function injectStateToChatArray(eventData) {
    ...
    var outLines = [
      ...
      '重要记忆点：',
      '- 角色名：记忆1|记忆2',
      '[追踪规则]：仅追踪女性角色，排除用户角色。最小变化！无明确证据的字段一律严格复制上一状态：时间按事件推进（对话几分钟/战斗几十分钟/旅行数小时天，无流逝则不变，用户给定以用户为准）；区域仅明确移动时更新；在场仅明确加入/离开/获得BUFF时更新（伤口、心态、衣服不算BUFF）；不在场仅随在场变化；处女膜仅破处性交时更新；做爱次数仅性交射精时+1；好感度仅明确变化时更新（有世界书系统则参照）；记忆仅新事件追加，每角色≤6条合计≤70字；外貌仅换装/脱衣/发型变化时更新，严禁凭空写裸体或换衣。若省略状态输出，世界状态将无法更新。]'
    ].join('\n');
  ...
  function buildSummarizePrompt(targetKind) {
    ...
    var p = '你是世界状态追踪器。根据「上一状态」和「聊天记录」，线性推演出当前最新状态。' +
      ...
      '聊天记录：\n' + hist + '\n\n' +
      '输出格式（每字段独占一行）：\n时间：\n区域：\n在场角色+BUFF：\n不在场角色：\n处女膜状态：\n做爱次数：\n当前好感度：\n身体外貌：\n重要记忆点：\n- 角色名：记忆1|记忆2\n\n' +
      '规则：仅追踪女性角色，只输出状态数据不续写故事。最小变化原则：无明确证据的字段必须严格复制上一状态！' +
      '时间精确到分钟，按事件推进（对话几分钟/战斗几十分钟/旅行数小时天，无流逝则不变）；' +
      '区域仅明确移动时更新；在场角色+BUFF仅明确加入/离开/获得BUFF时更新（伤口、心态、衣服不算BUFF）；' +
      '不在场仅随在场变化；处女膜仅破处性交时更新；做爱次数仅性交射精时+1；' +
