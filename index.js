(function () {
  ...
  function getDefaultFavorabilitySystem() {
    return '好感度范围-100~+100，分档：-100~-81仇敌/憎恨 | -80~-61厌恶/敌视 | -60~-41反感/疏远 | -40~-21冷淡/不满 | -20~-1警惕/不信任 | 0~20陌生/中立 | 21~40普通/友好 | 41~60信任/亲近 | 61~80依赖/喜欢 | 81~100深爱/忠诚。' +
           '评级掉落比增加更容易：负面事件/言行导致下降，正面事件/言行导致上升，但增加需更充分明确的正面证据。' +
           '典型影响：重大背叛/伤害/欺骗 -15~-30；日常不快/冷淡 -3~-10；日常友好/互动 +1~+5；重大恩情/告白/拯救 +8~+20。';
  }
  ...
  function normalizeSummaryLines(text) {
    // 一次性正则处理（长标签在前）：避免循环逐个替换时短标签二次破坏长标签子串
    // （如「在场角色：」是「不在场角色：」的子串，逐标签循环会把它拆成 不+在场角色：）
    var labels = [
      '不在场角色：', '不在场角色:',
      '在场角色\\+BUFF：', '在场角色\\+BUFF:',
      '在场角色：', '在场角色:',
    ...
    // 在每个字段标签前插入换行符（保持标签独占一行，行首多出的空行会被后续 trim 跳过）
  ...
  function parseSummary(text) {
    ...
    // 注意：长标签必须在短标签前面（如「不在场角色：」必须先于「在场角色：」，否则子串会被错误切分）
    var FIELD_LABELS = [
      '时间：', '时间:',
      '区域：', '区域:',
      '不在场角色：', '不在场角色:',
  ...
  function buildStatePrompt(state) {
    ...
    // 核心约束规则（v3.8.5 细项：最小变化原则 — 无明确证据一律严格复制上一状态）
    lines.push('');
    lines.push('[追踪规则]：');
    lines.push('0. 核心原则：最小变化！每个字段仅在本轮正文有明确证据时才更新，否则必须严格复制上一状态，禁止凭空修改。');
    lines.push('1. 仅追踪非用户女性角色：男性角色、魔王、NPC、性别不明的角色一律不出现在任何字段中。');
    if (userName) lines.push('2. 用户扮演的角色「' + userName + '」不追踪，任何字段不出现。');
    lines.push('3. 时间：精确到分钟（如：第3天 14:05）。在上一轮时间基础上按本轮事件推断推进：对话→几分钟，战斗→几十分钟，长途旅行→数小时或数天；若本轮无任何可感知的时间流逝则保持不变；用户消息中明确给出时间时以用户为准。');
    lines.push('4. 区域：仅当本轮正文中角色明确移动到新地点才更新，否则严格复制。层级格式：中国-湖北省-武汉市-武昌区-友谊大道-湖北大学 或 魔王城堡-卧室-床上。');
    lines.push('5. 在场角色+BUFF：仅当角色明确加入/离开/获得BUFF才更新，否则严格复制。格式：琴（回溯魔法）|芭芭拉。括号内只写BUFF（永久/临时正向/负向收益，如回溯魔法、中毒、力量增强）；禁止写身体状态（全裸、流血、伤口、昏睡、衣服破损等一律禁止）；无BUFF时只写角色名，不加括号。');
    lines.push('6. 不在场角色：仅当在场角色变化导致不在场列表变化时才更新。格式：角色名-在做什么（≤10字）。没有不在场角色时写「无」或「所有角色都在场」，禁止留空。');
    lines.push('7. 处女膜状态：仅当本轮正文明确发生破处性交才更新，否则严格复制。');
    lines.push('8. 做爱次数：仅当本轮正文明确发生性交且射精才+1，否则严格复制。');
    lines.push('9. 当前好感度：仅当角色言行明确体现好感变化才更新，否则严格复制；世界书有好感度系统时严格参照其计算。');
    lines.push('10. 重要记忆点：仅当发生值得记录的新事件才追加。每角色最多6条；出现第7条时与前面6条对比，替换最不重要的一条。每角色6条记忆合计≤70字（N个角色合计≤70N字）。');
    lines.push('11. 身体外貌：仅当明确描写换装/脱衣/发型变化才更新，否则严格复制；严禁凭空写裸体或换衣。格式：琴：琴骑士制服破损/金发散乱遮脸/下唇血痂干涸/指尖掐入掌心渗血/背靠大树瘫坐。');
    lines.push('12. 禁止泛称！不允许写"七神""众人""其他角色"等非具体角色名。');
    ...
    if (isFirstTimeState(state) && hasChatHistory()) {
      ...
      lines.push('请根据上方聊天历史，用纯文本输出当前世界的完整状态，9个字段每行一个（时间：/区域：/在场角色+BUFF：/不在场角色：/处女膜状态：/做爱次数：/当前好感度：/身体外貌：/重要记忆点：）。');
    }

    // 输出指令（方案A：纯文本格式，AI更容易遵循；禁止HTML注释/标签包裹）
    lines.push('');
    lines.push('[世界状态输出指令]：');
    lines.push('回复正文结束后，必须在末尾用纯文本追加当前世界状态，9个字段每行一个，直接输出字段名+值。');
    lines.push('禁止使用任何HTML注释、标签或<summary>包裹，禁止省略任何字段。');
    lines.push('正确示例（依据追踪规则的最小变化输出）：');
    lines.push('时间：第1天 15:30');
    lines.push('区域：蒙德城·天使的馈赠');
    lines.push('在场角色+BUFF：琴（回溯魔法）|芭芭拉（回溯魔法）');
    lines.push('不在场角色：所有角色都在场');
    lines.push('处女膜状态：完好');
    lines.push('做爱次数：0次');
    lines.push('当前好感度：友好(65/100)');
    lines.push('身体外貌：琴：琴骑士制服破损/金发散乱遮脸/下唇血痂干涸');
    lines.push('重要记忆点：');
  ...
  function injectStateToChatArray(eventData) {
    ...
    var wbData = getWorldBookData();
    var outLines = [
      '\n\n[世界状态输出指令]：回复正文结束后，必须在末尾用纯文本追加当前世界状态，9个字段每行一个，直接输出字段名+值，禁止使用任何HTML注释或标签，禁止省略任何字段：',
      '时间：',
      '区域：',
      '在场角色+BUFF：',
      '不在场角色：',
  ...
  function buildSummarizePrompt(targetKind) {
    ...
    var p = '你是世界状态追踪器。根据「上一状态」和「聊天记录」，线性推演出当前最新状态。' +
      ...
      '聊天记录：\n' + hist + '\n\n' +
      '输出格式（每字段独占一行）：\n时间：\n区域：\n在场角色+BUFF：\n不在场角色：\n处女膜状态：\n做爱次数：\n当前好感度：\n身体外貌：\n重要记忆点：\n- 角色名：记忆1|记忆2\n\n' +
      '规则：仅追踪非用户女性角色（男性/NPC/魔王一律不记录），只输出状态数据不续写故事。最小变化原则：无明确证据的字段必须严格复制上一状态！' +
      '时间精确到分钟，按事件推进（对话几分钟/战斗几十分钟/旅行数小时天，无流逝则不变）；' +
      ...
      '重要记忆仅新事件追加，每角色最多6条合计≤70字（出现第7条时替换最轻的一条）。';
    return p;
  }
