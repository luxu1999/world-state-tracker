(function () {
  ...
  var WST_VERSION = '3.8.4'; // 版本号：更新后首次使用自动清理旧数据
  ...
  function buildStatePrompt(state) {
    ...
    lines.push('正确示例（时间推进、好感上升后的状态输出）：');
    lines.push('时间：第1天 15:30');
    lines.push('区域：蒙德城·天使的馈赠');
    lines.push('在场角色+BUFF：琴(状态良好)|芭芭拉(休息中)');
    lines.push('不在场角色：诺艾尔-在骑士团值班');
    lines.push('处女膜状态：完好');
    lines.push('做爱次数：0次');
    lines.push('当前好感度：友好(65/100)');
    lines.push('身体外貌：及腰金发扎成高马尾，蓝瞳，白色骑士制服');
    lines.push('重要记忆点：');
    lines.push('- 琴：旅行者帮助她解决了风魔龙危机|她欠旅行者一个人情');
    lines.push('规则：时间 = 上一轮时间 + 本轮事件大致经历的时长；重要记忆点每人最多6条，每条不超过70字；新状态必须与上一状态不同（至少一个字段有可观察变化），变化必须符合本轮聊天内容的线性逻辑。');
  ...
  function injectStateToChatArray(eventData) {
    ...
    var outLines = [
      ...
      '重要记忆点：',
      '- 角色名：记忆1|记忆2',
      '注意：只追踪女性角色。时间=上一轮时间+本轮事件时长。重要记忆每人≤6条≤70字。若省略状态输出，世界状态将无法更新。]'
    ].join('\n');
